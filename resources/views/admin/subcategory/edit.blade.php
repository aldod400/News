                                            ...
                                            <select class="form-control" id="parent_category_id" name="parent_category_id" required>
                                                ...
                                                @foreach($allCategories as $category)
                                                    <option value="{{ $category->id }}" 
                                                        {{ $subCategory->parent_category_id == $category->id ? 'selected' : '' }}>
                                                        {{ $category->name }}
                                                    </option>
                                                @endforeach
                                            </select>
                                    ...
                                    <div class="col-md-6">
                                        <div class="form-group">
                                            ...
                                            <input type="text" class="form-control" id="name" name="name" 
                                                   placeholder="Enter sub category name" value="{{ $subCategory->name }}" required>
                                        </div>
                                    </div>
                                </div>
                                <div class="row">
            ...
            $.ajax({
                ...
                success: function(response) {
                    if (response.success) {
                        ...
                        submitButton.prop('disabled', false).html('<i class="fa fa-save"></i> Update Sub Category');
                    }
                },
                error: function(xhr) {
